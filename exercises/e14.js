    ...
    if(!withdrawals){
      sumOfWithdrawals = 0}
        ...
        for(let k = 0; k < withdrawals.length; k++){
          sumOfWithdrawals += withdrawals[k]
    		}
  	}
   if((sumOfClientDeposits - sumOfWithdrawals) !== curCust.balance  ) {
     outOfBalAccounts.push(curCust)
   }
  }
  return outOfBalAccounts
}
